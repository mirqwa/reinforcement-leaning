...
def sarsa(sim_input, sim_output) -> (np.array, list):
    ...
    for episode in range(num_episodes):
        ...
        while not game_over:
            ...
            # Select next action which should always be greedy since we want the max of the
            # next state action values
            next_greedy_action = actions.epsilon_greedy_action(next_state, q_table, 0)

            # Determine Q-value next state (on-policy)
            next_greedy_state_value = q_table[next_greedy_action][next_state]

            # Update Q-table
            q_table = qtable.update_q_table(
                q_table, state, action, reward, next_greedy_state_value, gamma, alpha
            )
